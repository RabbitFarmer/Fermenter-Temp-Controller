...
def batch_history():
    """
    List all batch files with their metadata for selection.
    """
    batches = []
    ...
    if os.path.exists(BATCHES_DIR):
        for filename in os.listdir(BATCHES_DIR):
            ...
            # Read first batch_metadata event from the file
            try:
                with open(filepath, 'r') as f:
                    for line in f:
                        ...
                        try:
                            ...
                            # Count samples
                            if obj.get('event') == 'sample':
                                batch_info['sample_count'] += 1
                            
                            # Extract metadata from first batch_metadata event
                            if obj.get('event') == 'batch_metadata' and batch_info['brewid'] is None:
                                payload = obj.get('payload', {})
                                meta = payload.get('meta', {})
                                batch_info['brewid'] = payload.get('brewid')
                                batch_info['tilt_color'] = payload.get('tilt_color')
                                batch_info['created_date'] = payload.get('created_date')
                                batch_info['beer_name'] = meta.get('beer_name')
                                batch_info['batch_name'] = meta.get('batch_name')
                        except json.JSONDecodeError:
                            continue
            except Exception as e:
                print(f"[LOG] Error reading batch file {filename}: {e}")
                continue
            
            # Only include batches with valid metadata
            if batch_info['brewid']:
                batches.append(batch_info)
    
    # Sort by created_date descending (most recent first)
    batches.sort(key=lambda x: x.get('created_date') or '', reverse=True)
    
    return render_template('batch_history.html',
        batches=batches,
        system_settings=system_cfg,
        COLOR_MAP=COLOR_MAP
    )
...
@app.route('/batch_history/<brewid>')
def batch_history_detail(brewid):
    """
    Show detailed view of a specific batch with plotly chart.
    """
    # Find the batch file
    batch_file = None
    if os.path.exists(BATCHES_DIR):
    ...
    if not batch_file:
        abort(404, description=f"Batch {brewid} not found")
    
    # Read batch metadata and samples
    metadata = {
        'brewid': brewid,
        'beer_name': 'Unknown',
        'batch_name': 'Unknown',
    ...
    try:
            ...
            for line in f:
                ...
                try:
                    ...
                    if obj.get('event') == 'batch_metadata':
                        payload = obj.get('payload', {})
                        meta = payload.get('meta', {})
                        metadata['brewid'] = payload.get('brewid', brewid)
                        metadata['tilt_color'] = payload.get('tilt_color', 'Unknown')
                        metadata['created_date'] = payload.get('created_date', 'Unknown')
                        metadata['beer_name'] = meta.get('beer_name', 'Unknown')
                        metadata['batch_name'] = meta.get('batch_name', 'Unknown')
        ...
        print(f"[LOG] Error reading batch file {batch_file}: {e}")
        abort(500, description=f"Error reading batch data: {e}")
    
    return render_template('batch_history_detail.html',
        metadata=metadata,
        samples=samples,
        system_settings=system_cfg
    )
