...
This is a full replacement app.py that:
- Removes any direct SmartPlug import (kasa control runs in the kasa_worker process).
- Starts a kasa_worker process (multiprocessing.Process) with command/result queues.
- Provides the /api/live_snapshot endpoint used by the UI polling script for quick updates.
- Keeps BLE scanning, periodic temperature control loop, kasa result listener, and routes.
- Ensures update_temp_config route matches the templates ("/update_temp_config").
"""
...
# IPC queues for kasa worker
kasa_queue = Queue()
kasa_result_queue = Queue()

# Start kasa worker process (daemon so it exits with the parent)
kasa_proc = Process(target=kasa_worker, args=(kasa_queue, kasa_result_queue))
kasa_proc.daemon = True
kasa_proc.start()

# In-memory data structures (persisted config is in files)
tilt_cfg = {}
temp_cfg = {}
system_cfg = {}
live_tilts = {}
...
# Ensure expected keys exist in temp_cfg
...
def log_tilt_reading(color, gravity, temp_f, rssi):
    ...
    # Optionally persist to a file or database if desired
...
# --- Temperature control and plug commands --------------------------------
...
def send_sms(body):
    print(f"[LOG] SMS: {body}")  # placeholder
...
def control_heating(state):
    ...
    if not enabled or not url:
        ...
        print("[LOG] Heating control bypassed (not enabled or no URL). heater_on/pending set to False")
        return
    ...
    print(f"[LOG] control_heating queued state={state}, pending set to True, url={url}")
...
def control_cooling(state):
    ...
    if not enabled or not url:
        ...
        print("[LOG] Cooling control bypassed (not enabled or no URL). cooler_on/pending set to False")
        return
    ...
    print(f"[LOG] control_cooling queued state={state}, pending set to True, url={url}")
...
# --- Kasa result listener (updates flags on confirmation from worker) -------
...
def kasa_result_listener():
    ...
    while True:
        try:
            ...
            if mode == 'heating':
                temp_cfg["heater_pending"] = False
                temp_cfg["heater_on"] = bool(success and action == 'on')
                temp_cfg["heating_error"] = not success
                temp_cfg["heating_error_msg"] = result.get('error', '') or ''
            elif mode == 'cooling':
                temp_cfg["cooler_pending"] = False
                temp_cfg["cooler_on"] = bool(success and action == 'on')
                temp_cfg["cooling_error"] = not success
                temp_cfg["cooling_error_msg"] = result.get('error', '') or ''
            else:
                print(f"[LOG] Unknown mode in kasa result: {mode}")
        except Exception:
            # timeout or other error, loop back
            continue

threading.Thread(target=kasa_result_listener, daemon=True).start()

# --- Periodic temperature control thread -----------------------------------

def periodic_temp_control():
    while True:
        try:
            # reload latest config from disk (so web UI changes apply without restart)
...
def update_temp_config():
    ...
    print(f"[LOG] Received form data in update_temp_config:", dict(data))
    try:
        temp_cfg.update({
    ...
    print(f"[LOG] Updated temp_cfg in update_temp_config:", temp_cfg)
    try:
    ...
    # Apply control logic immediately
    temperature_control_logic()
...
def update_system_config():
    ...
    print(f"[LOG] Updated and saved system_cfg in update_system_config:", system_cfg)
...
def scan_kasa_plugs():
    ...
    devices = {}
    try:
        found_devices = __import__('asyncio').run(Discover.discover())
...
# --- Live snapshot endpoint for client polling -----------------------------
...
# --- Simple temp report / export endpoints (stubs) -------------------------

@app.route('/temp_report', methods=['POST'])
def temp_report():
    color = request.form.get('tilt_color')
    # Implementation of report generation would go here. For now, redirect to a simple selector.
    return redirect('/temp_config')
...
if __name__ == '__main__':
    # Run the Flask debug server (use a production server for real deployments)
